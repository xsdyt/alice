...
class AppsController extends Controller {
	public function anyConfig()
	{
		...
		if($config_id!='')
		{
			$rows = DB::connection('conf')->select('call sp_get_config_via_id(?);',[$config_id]);
		}
		else
		{
			...
			$rows = DB::connection('conf')->select('call sp_get_config(?,?,?,?,?,?,?,?);',[$uuid,$store,$platform,$channel,$version,$os,$countryCode,$address]);
		}
		...
		if(count($rows)>0)
		{
			...
			if($config->config_run_mode==9 && ($address=='101.231.108.131' || $address=='106.49.97.250'))
				$config->config_run_mode = 1;
			
			$log = LogModel::createSessionLog($config->config_id,$platform,$adid,'', $version, $address, $imei, $uuid, $device, $os, $network,$config->config_id);
			if (count($log) > 0) {
				$config->log_id = $log[0]->log_id;
			}
			
			$result = json_encode($config,JSON_UNESCAPED_UNICODE|JSON_NUMERIC_CHECK);
			Log::info('config result config_id['.$config->config_id.'] config_name['.$config->config_name.']');
		}
		else
		{
